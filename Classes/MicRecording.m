...
- (float) averagePower
{
    ...
    if (status) {NSLog(@"Error retrieving meter data\n"); return 0.0f;}
...
- (void) stopRecording
{
    [self performSelector:@selector(reallyStopRecording) withObject:NULL afterDelay:0.1f];
}

- (void) reallyPauseRecording
{
	if (!recordState.queue) {NSLog(@"Nothing to pause\n"); return;}
...
- (BOOL) resume
{
    if (!recordState.queue){NSLog(@"Nothing to resume\n"); return NO;}
    OSStatus status = AudioQueueStart(recordState.queue, NULL);
    if (status) {NSLog(@"Error restarting audio queue\n"); return NO;}
    return YES;
}
...
- (float) currentTime
{
    ...
    if (status) {NSLog(@"Error: Could not retrieve current time\n"); return 0.0f;}
